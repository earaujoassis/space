...
func (s *ApiHandlerTestSuite) TestUsersClientsListHandlerWithoutHeader() {
...
func (s *ApiHandlerTestSuite) TestUsersClientsListHandlerByUnauthenticatedUser() {
	header := &http.Header{
...
func (s *ApiHandlerTestSuite) TestUsersClientsListHandlerWithoutActionGrant() {
	header := &http.Header{
		"X-Requested-By": []string{"SpaceApi"},
	}
...
func (s *ApiHandlerTestSuite) TestUsersClientsListHandlerByAnotherUser() {
	cookie := s.createSessionCookie(true)
	s.NotNil(cookie)
...
func (s *ApiHandlerTestSuite) TestUsersClientsListHandlerInvalidId() {
	cookie := s.createSessionCookie(true)
	s.NotNil(cookie)
	user := s.Factory.GetAvailableUser()
	actionToken := s.Factory.NewAction(user).Model.Token
...
func (s *ApiHandlerTestSuite) TestUsersClientsListHandler() {
	cookie := s.createSessionCookie(true)
	s.NotNil(cookie)
	user := s.Factory.GetAvailableUser()
	actionToken := s.Factory.NewAction(user).Model.Token
