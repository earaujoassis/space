...
func (s *ApiHandlerTestSuite) TestApplicationBootstrapHandler() {
	header := &http.Header{
		"X-Requested-By": []string{"SpaceApi"},
	}
	w := s.PerformRequest(s.Router, "GET", "/api/application/bootstrap", nil, nil, nil)
	...
	w = s.PerformRequest(s.Router, "GET", "/api/application/bootstrap", header, nil, nil)
	r = utils.ParseResponse(w.Result(), nil)
	s.Require().Equal(401, w.Code)
	s.Contains(r.Body, "User must be authenticated")
	cookie := s.createSessionCookie(false)
	s.NotNil(cookie)
	w = s.PerformRequest(s.Router, "GET", "/api/application/bootstrap", header, cookie, nil)
	r = utils.ParseResponse(w.Result(), nil)
