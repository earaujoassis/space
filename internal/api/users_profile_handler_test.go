...
func (s *ApiHandlerTestSuite) TestUsersProfileHandlerWithoutHeader() {
...
func (s *ApiHandlerTestSuite) TestUsersProfileHandlerByUnauthenticatedUser() {
	header := &http.Header{
...
func (s *ApiHandlerTestSuite) TestUsersProfileHandlerWithoutActionGrant() {
	header := &http.Header{
		"X-Requested-By": []string{"SpaceApi"},
	}
...
func (s *ApiHandlerTestSuite) TestUsersProfileHandlerByAnotherUser() {
	cookie := s.createSessionCookie(true)
	s.NotNil(cookie)
...
func (s *ApiHandlerTestSuite) TestUsersProfileHandlerInvalidId() {
	cookie := s.createSessionCookie(true)
	s.NotNil(cookie)
	user := s.Factory.GetAvailableUser()
	actionToken := s.Factory.NewAction(user).Model.Token
...
func (s *ApiHandlerTestSuite) TestUsersProfileHandler() {
	cookie := s.createSessionCookie(true)
	s.NotNil(cookie)
	user := s.Factory.GetAvailableUser()
	actionToken := s.Factory.NewAction(user).Model.Token
