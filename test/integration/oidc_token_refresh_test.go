...
func (s *OIDCProviderSuite) TestTokenRefreshGrant() {
	...
	s.Run("should return error if attempting to refresh session token through another client", func() {
		...
		response := s.Client.PostTokenRefresh(second_client.BasicAuthEncode(), refreshToken, "openid")
		json := response.JSON
	...
	s.Run("should return error if attempting to refresh session token with the access session token", func() {
		response := s.Client.PostTokenRefresh(client.BasicAuthEncode(), accessToken, "openid")
		json := response.JSON
	...
	s.Run("should return error if extending scope previously requested", func() {
		response := s.Client.PostAuthorize(Code, client.Key, "http://localhost/callback", "test-state", true)
		...
		response = s.Client.PostTokenRefresh(client.BasicAuthEncode(), refreshToken, "openid profile")
		json = response.JSON

		s.Equal(400, response.StatusCode)
		s.Equal("invalid_scope", json["error"])
	})
	...
	s.Run("should return success if all parameters are correct", func() {
		...
		response = s.Client.PostTokenRefresh(client.BasicAuthEncode(), refreshToken, "openid")
		json = response.JSON
