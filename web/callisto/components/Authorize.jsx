...
const Authorize = () => {
  ...
  const requestedData = scope => {
    switch (scope) {
      case 'openid':
        return ['Authentication data for that given application']
      case 'public':
        return ['Authentication data for that given application']
      case 'profile':
        return [
    ...
    <div className="authorize">
      ...
      <Row className="separator">
        <Columns className="small-offset-2 small-8 end">
          <p>
            The application ({serverData.client_name}) is requesting access to
            the following information:
          </p>
          <ul className="">
            {requestedData(
              getHighestWeightScope(serverData.requested_scope)
            ).map(message => {
              return <li key={keyNumber++}>{message}</li>
            })}
          </ul>
          <p>
            {serverData.client_name} (the application) and Space (the current
            website) will use this information in accordance with their
          ...
          <Row>
            <Columns className="small-offset-2 small-4">
              <form action="" method="post">
                ...
                <button className="button expand secondary" type="submit">
                  Cancel
                </button>
              </form>
            </Columns>
