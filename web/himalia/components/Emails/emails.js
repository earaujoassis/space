            ...
            {emails.map((entry) => (
                <li key={entry.id || 'primary-email'}>
                    <div className="emails__entry">
                        <p>
                            <span className="emails__entry-address">
                                {entry.address}
                            </span>
                        ...
                        <p className="emails__entry-action">
                            {entry.verified || requestedVerification.includes(entry.id) ? null : (
                                <button
                                    onClick={(e) => {
                                        e.preventDefault()
                                        requestEmailVerification(entry.address)
                                        setRequestedVerification([entry.id, ...requestedVerification])
                                    }}
